...
public class EnviSetup {
	...
	public static void main(String[] args) {
		...
		try {
			HashMap<String, Object> argMap = setupArgs(args, log);
			...
			SetupDriver dv = new SetupDriver(argMap, log);
			dv.runSetup();
	...
	private static HashMap<String, Object> setupArgs(String[] args, Log log) 
		...
		.help("Ending chromosome number. Must be equal to or greater than starting chromosome number.");
		...
		parser.addArgument("-wd", "--working_dir").type(Arguments.fileType().verifyIsDirectory()
				...
				.help("Directory for output. If not set, defaults to the current working directory");
		...
		{
			e.printStackTrace();
			throw new IllegalInputException (log, "Error: Failed to parse arguments.\nError: " + 
					e.getMessage() + "\n");			
		}	    
	    ...
	    {
	    	throw new IllegalInputException(log, "Error: End Chromosome is before start chromosome");
	    }
	    ...
	    {
	    	throw new IllegalInputException(log, "Error: Cross population is same as target population");
	    }
	    else if (parsedArgs.get("out_pop").equals(parsedArgs.get("target_pop") ) )
	    {
	    	throw new IllegalInputException(log, "Error: Out-group population (-op) is same as target population");
	    }
